          ...
          <tr v-for="(row, idx) in rows" :key="idx" :class="{ existing: row.isExisting }">
            ...
            <td>
              <select v-model="row.type" @change="markDirty">
                ...
                <option :value="logsStore.logType('MAINT')">Maintenance (2x)</option>
                <option :value="logsStore.logType('SETUP')">Trial Setup (2x)</option>
              </select>
            </td>
            <td>
...
watch(() => props.sheet, async (newSheet) => {
  ...
  // Map to the standardized string from the store
...
const addRow = () => {
  ...
  rows.value.push({ 
    ...
    Activity: '', 
    clockHours: 0, 
    // Use the store's default string
    type: logsStore.logType('STANDARD') 
  })
}
