  ...
  <div class="flex h-screen bg-gray-100 font-sans">
    <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col shadow-xl print:hidden">
      ...
      <div class="p-6 border-b border-slate-800">
        ...
        <div v-if="authStore.profile" class="mt-4 flex items-center gap-3 bg-slate-800/50 p-2 rounded-lg border border-slate-700/50">
      ...
      <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
        <RouterLink to="/dashboard" active-class="bg-indigo-600 text-white shadow-lg shadow-indigo-900/50" class="flex items-center px-4 py-3 rounded-lg transition-all hover:bg-slate-800 hover:text-white group mb-4">
        ...
        <RouterLink to="/meeting" active-class="bg-slate-800 text-white border-l-4 border-indigo-500" class="flex items-center px-4 py-2 rounded-r-lg transition-colors hover:bg-slate-800 hover:text-white group text-sm">
      ...
      <div class="p-4 border-t border-slate-800 space-y-2">
        <RouterLink to="/import" class="flex items-center px-4 py-2 text-xs text-yellow-600 hover:text-yellow-500 hover:bg-slate-800 rounded transition-colors">
          <span class="mr-2">⚠️</span> Data Importer
        </RouterLink>
        
        <button @click="handleLogout" class="w-full flex items-center px-4 py-2 text-xs text-slate-400 hover:text-white hover:bg-slate-800 rounded transition-colors text-left">
